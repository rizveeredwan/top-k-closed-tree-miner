...
class KCloTreeMiner:
    ...
    def patterns_quality_check(self, pattern, support, cspm_tree_nodes, cspm_tree_node_bitset, NODE_MAPPER):
        ...
        if self.support_table.get(support) is not None:
            ...
            """
            # trie version 
            #L1, L2 = closure_check_with_trie(trie_head=self.support_table[support].closed_patterns_with_trie[0], p=pattern, type_flag=0)
            """
            if len(L1) > 0 and len(L2) > 0:
                ...
                for i in range(0, len(L2)):
                    if VERDICT is True:
                        if (str(L2[i].pattern) == str([[3], [9], [6], [8]])):
                            DEBUG_VERDICT = True
                    # old version
                    self.support_table[support].closed_patterns.delete(current=L2[i])
                if VERDICT is True and DEBUG_VERDICT is False:
                    print(f"problem {pattern}")
                    debug_functions.print_pattern_from_ll_nodes(_list=L2)
                    print(debug_functions.searching_a_pattern_in_ll(start=self.support_table[support].closed_patterns,
                                                                    pattern=[[3], [9], [6], [8]]))

                # print(f"super = {pattern}") # debug
                # new trie version
                # self.support_table[support].remove_closed_pattern(support_table_entry=self.support_table[support], node_to_delete=L2[i])
            ...
            if candidacy_flag is True:
                ...
                L1, L2 = closure_check(linked_list_nodes=self.support_table[support].caphe_node.pattern_ll_node[0],
                                       p=pattern)
                """
                L1, L2 = closure_check_with_trie(trie_head=self.support_table[support].caphe_node.pattern_ll_node[0],
                                                 p=pattern, type_flag=1)
    ...
    def check_only_in_candidates(self, pattern, support, cspm_tree_nodes, cspm_tree_node_bitset, NODE_MAPPER):
        # checking only in candidates, if pattern can be closed/ is it removing some closed / abosortion status
        candidacy_flag, closed_flag = True, None
        if self.support_table.get(support) is not None:
            # checking with closed patterns
            # old version
            ...
            if len(L1) > 0:
                ...
                if absorption_status[0] is True:
                    candidacy_flag = False
            if len(L2) > 0:  # P has enclosed some
                for i in range(0, len(L2)):
                    # old version
                    self.support_table[support].closed_patterns.delete(current=L2[i])
        work_with_sex = candidacy_flag  # True: need sex, False: no need for sex
        flag = closed_flag  # None: closed, False: not closed
    ...
    def k_clo_tree_miner(self, cspm_tree_root, K=2, NODE_MAPPER=None):
        ...
        while len(self.caphe.nodes) > 0:
            ...
            print(
                f"***************** SHURUR SHOB CANDIDATES : {ITR_CNT} {self.return_current_possible_minsup(K)} {len(self.support_min_heap)}*************")
            # debug_functions.print_support_min_heap(self.support_min_heap)
            # print("current ", caphe_node.support)
            # debug_functions.print_all_the_candidates(self.support_table)
            # debug_functions.print_all_the_closed(self.support_table)
            ...
            if output is None:  # all the patterns have been checked
                ...
                work_with_sex, flag = self.check_only_in_candidates(pattern=pattern, support=support,
                                                                    cspm_tree_nodes=cspm_tree_nodes,
                                                                    cspm_tree_node_bitset=cspm_tree_node_bitset,
                                                                    NODE_MAPPER=NODE_MAPPER)
                ...
                if flag is None:  # this pattern can be closed (0 means not closed), update that information
                    """
                    self.support_table[support].add_closed_pattern(support_table_entry=self.support_table[support],
                                                                   pattern=pattern, cspm_tree_nodes=cspm_tree_nodes,
                                                                   cspm_tree_node_bitset=cspm_tree_node_bitset)
                    # debug_functions.print_all_the_closed(self.support_table, specific=support)
                    """
