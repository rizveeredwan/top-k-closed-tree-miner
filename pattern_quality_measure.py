...
def absorption_check(list_of_ll_nodes, nodes_of_p, flag, NODE_MAPPER=None):
    # flag 0: list_of_ll_nodes have absorbed pattern p
    # flag 1: p has absorbed list_of_ll_nodes
    absorption_status = []
    for i in range(0, len(list_of_ll_nodes)):
        cpsm_tree_nodes = []
        if list_of_ll_nodes[i].cpsm_tree_nodes is not None:
            cpsm_tree_nodes = list_of_ll_nodes[i].cpsm_tree_nodes
        elif list_of_ll_nodes[i].cpsm_tree_node_bitset is not None:
            cpsm_tree_nodes = generate_cspm_tree_nodes_from_bitset(
                cspm_tree_node_bitset=list_of_ll_nodes[i].cpsm_tree_node_bitset,NODE_MAPPER=NODE_MAPPER)
        if flag == 0:
            verdict = two_pattern_absorption_check(nodes_of_A=cpsm_tree_nodes, nodes_of_B=nodes_of_p)
        elif flag == 1:
            verdict = two_pattern_absorption_check(nodes_of_A=nodes_of_p, nodes_of_B=cpsm_tree_nodes)
